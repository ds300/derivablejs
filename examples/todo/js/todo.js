...
var _havelock = require('havelock');
...
function cursor() {
  ...
  return {
    ...
    set: function set(x, v) {
      return x.setIn(path, v);
    }
  };
}

function explode(derivableListT, idF) {
  idF = idF || function (x) {
    return x;
  };
  var ids = derivableListT.derive(function (list) {
    return list.map(idF).toList();
  });

  var cache = _immutable2['default'].Map();

  var id2idx = ids.derive(function (ids) {
    var startTime = +new Date();
    var map = _immutable2['default'].Map();
    ids.forEach(function (id, idx) {
      var idxs = map.get(id) || _immutable2['default'].List();
  ...
  return ids.derive(function (ids) {
    var startTime = +new Date();
    var newCache = _immutable2['default'].Map();
    var result = ids.map(function (id) {
      var itemDs = cache.get(id);
      var itemD = null;
      ...
      if (itemDs) {
        ...
        var idxD = id2idx.derive(function (map) {
          return map.get(id).get(itemD.idxidx);
        });
        itemD = (0, _havelock.derive)(derivableListT, idxD, function (xs, i) {
          return xs.get(i);
        });
      }

      var idAcc = newCache.get(id) || _immutable2['default'].List();
...
function innerMap(derivableListDerivableT, fn) {
  return derivableListDerivableT.derive(function (dxs) {
    return dxs.map(function (x, i) {
      return x.derive(function (x) {
        return fn(x, i);
      });
    });
  });
...
function dmap(fn, derivableListT) {
  return innerMap(explode(derivableListT), fn).derive(function (xs) {
    return xs.map(_havelock.get).toList();
  });
}

},{"havelock":3,"immutable":4}],2:[function(require,module,exports){
...
var _havelock2 = _interopRequireDefault(_havelock);

var _immutable = require('immutable');

var _immutable2 = _interopRequireDefault(_immutable);

var _batteries = require('./batteries');
...
var todos = (0, _havelock.atom)(_immutable2['default'].List());
var showing = (0, _havelock.atom)("all");

function newTodo(todos, description) {
  if (description.trim().length === 0) {
    return todos;
  } else {
    return todos.push((0, _immutable.fromJS)({ description: description, complete: false, editing: false }));
  }
}
...
function toggleComplete(todos, idx) {
  return todos.updateIn([idx, 'complete'], function (x) {
    return !x;
  });
}
...
function markAll(todos, complete) {
  return todos.map(function (t) {
    return t.put('complete', complete);
  });
}

/*** Derived data for rendering stuffs ***/

var size = todos.derive(function (ts) {
  return ts.size;
});

var numRemaining = todos.derive(function (ts) {
  return ts.filter(function (t) {
    return !t.complete;
  }).size;
});

var allComplete = size.and(numRemaining.not());

/*** Barebones React Component for Rendering Derivables ***/

var Derivable = _react2['default'].createClass({
  displayName: 'Derivable',
  ...
  getInitialState: function getInitialState() {
    ...
    return {
      ...
      reaction: this.props.render.reaction(function (vdom) {
        _this.setState({ vdom: vdom });
      })
    };
  },
  ...
  return _react2['default'].createElement(
    ...
    _react2['default'].createElement(
      ...
      { href: '', onClick: function (ev) {
          todos.swap(deleteTodo, todo, idx);ev.preventDefault();
        } },
      'x'
    )
  );
...
var appRender = todos.derive(function (todos) {
  return _react2['default'].createElement(
    'div',
    { 'class': 'buns' },
    _react2['default'].createElement('input', { type: 'text', onKeyPress: handleKeyPress, placeholder: 'what should i do?' }),
    todos.map(renderTodo).toArray()
  );
});

var elem = _react2['default'].createElement(Derivable, { render: appRender });

_react2['default'].render(elem, document.getElementById("main"));
...
},{"./batteries":1,"havelock":3,"immutable":4,"react":159}]},{},[160]);
